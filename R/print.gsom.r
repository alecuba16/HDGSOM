...
print.gsom <- function(x, ...){
  ...
  if(!is.null(x$mapped)){
    ...
    return(cat("HDGSOM object.", observations, "observations are mapped onto an existing GSOM map.\n"))
  }
  
  if(!is.null(x$prediction)){
    observations = length(x$prediction$bmn)
    return(cat("HDGSOM object. For", observations, "observations Y values were predicted according to an existing gsom object.\n"))
  }
  
  if(x$nhood == "rect") topology = "rectangular"
  else topology = "hexagonal"
  
  cat("HDGSOM (gsom) map with", nrow(x$nodes$position), "nodes and a", topology, "topology.\n")
