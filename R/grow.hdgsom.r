...
grow.hdgsom <- function(hdgsom_model, df, repet, spreadFactor, alpha, beta, gridsize, nhood, grow, initrad){
 ...
 initnodes <- gridsize*gridsize
  ...
  hdgsom_model <- list(nodes = nodes, training = training, GT = GT, norm_param=0, nhood = nhood)
  class(hdgsom_model) <- "hdgsom"
