#######################################
# grow_xy.hdgsom - HDGSOM
# Alex Hunziker - 2017
# Alejandro Blanco Martinez - 2018 (contribution)
#######################################

# The Functions in this File are required in order to train the hdgsom model.
# hdgsom.train() is the main function, which should be called by the user.

#Mainly calls the C loop and processes returned data
grow_xy.hdgsom <- function(y, df, repet, spreadFactor, alpha, beta, gridsize, nhood, grow, initrad){
  ...
  gt = -log(ncol(df)) * log(spreadFactor) * 0.01*nrow(df)
  ...
  hdgsom_object <- list(nodes = nodes, training = training, GT = GT, norm_param=0, nhood = nhood)
